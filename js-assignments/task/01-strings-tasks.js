...
function removeFirstOccurrences(str, value) {
  //throw new Error("Not implemented");
  let i = 0;
  const forward = value.length;
  var result = "";
  while (i < str.length - forward) {
    //console.log(str.slice(i, i + forward));
    if (str.slice(i, i + forward) != value) {
      result = result + str[i];
    } else {
      i = i + forward;
      break;
...
function getRectangleString(width, height) {
  ...
  let str = "";
  let str1 =
    leftangleup + horisChar.repeat(width - 2) + rigthangleup + breakChar;
  let str2 = "";
  for (let i = 0; i < height - 2; i++) {
    str2 += verticChar + whitespace.repeat(width - 2) + verticChar + breakChar;
  }
  let str3 =
    leftanglelow + horisChar.repeat(width - 2) + rigthanglelow + breakChar;
  str = str1 + str2 + str3;
  return str;
}
...
function encodeToRot13(str) {
  //throw new Error("Not implemented");
  let str1 = "NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm";
  let str2 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
  return str.replace(/[A-Z]/gi, (c) => str1[str2.indexOf(c)]);
}
...
function isString(value) {
  //throw new Error("Not implemented");
  let type_of_value = typeof value;
  if (type_of_value == "string") return true;
